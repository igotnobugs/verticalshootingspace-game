...
void main()
{
	...
	float linearDistance = length(frag_LightOrigin - baseDistance);
	float attenuation = 1.0 / (frag_LightAttenuation * linearDistance);
	vec4 color = vec4(attenuation, attenuation, attenuation, 1.0) * vec4(frag_LightColor, 1.0);
